...
export function TitleWithDivider({ title, titleStyle = {}, divider, className = "mb-8" }) {
  ...
  return (
    <div className={className} style={{ display: "flex", justifyContent: justifyMap[align] || "flex-start" }}>
      {/*
        inline-block container: its width = text width, height = text height.
        All % on the absolute bar are relative to this box.
        Bar: left=11%, width=122% → right edge at 133% (33% overshoot right).
        Bar: top=54%, height=calc(46%+3px) → covers bottom ~46% + dips 3px below baseline.
      */}
